...
class CustomDP(nn.Module):
    ...
    def forward(self, images, targets: Batch = None):
        ...
        if self.is_backbone:
            return self.gather(outputs, self.out_device, train=self.model.training)

        return self.gather(outputs, self.out_device, voxel_only=self.model.voxel_only,
                           backbone_train=self.model.backbone.training, train=self.model.train)
